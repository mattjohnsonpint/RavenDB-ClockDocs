...
namespace Raven.Bundles.ClockDocs.Tests
{
    public class ClockDocsTests : RavenTestBase
    {
        protected override void ModifyConfiguration(RavenConfiguration configuration)
        {
            ClockDocsHelper.RegisterBundle(configuration);
        }
        ...
        public void ClockDocsBundle_Creates_The_Clock_Doc()
        {
            using (var documentStore = NewDocumentStore())
            {
                ...
                using (var session = documentStore.OpenSession())
                {
                    var clock1 = session.Load<Client.Bundles.ClockDocs.Clock>(Client.Bundles.ClockDocs.Clock.IdBase + "EverySecond");
                    var clock2 = session.Load<Client.Bundles.ClockDocs.Clock>(Client.Bundles.ClockDocs.Clock.IdBase + "EveryFiveSeconds");
